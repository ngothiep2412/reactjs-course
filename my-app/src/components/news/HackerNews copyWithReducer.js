...
const hackNewsRuducer = (state, action) => {
  switch (action.type) {
    case "SET_DATA": {
...
const HackerNewsWithReducer = () => {
  const [state, dispatch] = React.useReducer(hackNewsRuducer, initialState);
  ...
  handleFetchData.current = async () => {
    ...
    try {
      ...
      dispatch({
        type: "SET_ERROR",
        payload: `The error was happeing ${error}`,
      });
      // setLoading(false);
